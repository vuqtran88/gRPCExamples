...
namespace GrpcServiceExample
{
	public class WeatherService : WeatherForecasts.WeatherForecastsBase
	{
        ...
        public override async Task GetWeatherStream(IAsyncStreamReader<RequestContext> requestStream, IServerStreamWriter<WeatherData> responseStream, ServerCallContext context)
        {
            while (await requestStream.MoveNext() && !context.CancellationToken.IsCancellationRequested)
            {
                ...
                if (current != null)
                {
                    if (current.Condition == "FailedPrecondition")
                    {
